...
wrangle_devices <- function(filtered_devices) {
    ...
    filtered_devices <-
      readr::read_csv(
        ...
        col_types = readr::cols(
          device_owner = col_character(),
          #  good to avoid ambiguity in case of sparce or odd dataset.
          device_id = col_character(),
          data_type = col_character(),
          comms_type = col_character(),
          long = col_double(),
          lat = col_double(),
          locationCode = col_character(),
  ...
  devices <-
    ...
    dplyr::filter((data_type == "CD" |
                     data_type == "TH")) %>%   # Sensor Node
    # Rental Type == 'council' / 'private'
    tibble::add_column(rental_type = "", .after = "cluster") %>%
    dplyr::mutate(rental_type = dplyr::case_when(
      grepl("council", cluster, fixed = TRUE) ~ "council",
      grepl("private", cluster, fixed = TRUE) ~ "private"
    )) %>%
    ...
    dplyr::mutate(building_type = dplyr::case_when(
      grepl("bungalow", cluster, fixed = TRUE) ~ "bungalow",
      grepl("flat", cluster, fixed = TRUE) ~ "flat",
      grepl("house", cluster, fixed = TRUE) ~ "house"
    )) %>%
    ...
    dplyr::mutate(room_type = dplyr::case_when(
      grepl("Bedroom", room, fixed = TRUE) ~ "bedroom",
      grepl("Lounge",  room, fixed = TRUE) ~ "living",
      grepl("Living",  room, fixed = TRUE) ~ "living"
    )) %>%
